...
final class OrderCreationService
{
    ...
    public function create(CreateOrderInput $input): array
    {
        ...
        $this->assertSafeNotifyUrl($input->notifyUrl);
        $this->assertSafeReturnUrl($input->returnUrl);
    ...
    private function assertSafeNotifyUrl(string $url): void
    {
        if ($url === '') {
            return;
        }
        ...
        try {
            UrlSafety::assertPublicHttpTarget($url, '回调地址不合法');
        } catch (\InvalidArgumentException $e) {
            if (in_array($e->getMessage(), ['URL 不允许使用内网地址', 'URL 解析到内网地址'], true)) {
    ...
    private function assertSafeReturnUrl(string $url): void
    {
        if ($url === '') {
            return;
        }
        ...
        if (!$parts || !in_array($scheme, ['http', 'https'], true) || $host === '') {
            throw new ValidationException('返回地址不合法');
        }
        if ($scheme !== 'https') {
            throw new ValidationException('返回地址必须使用 HTTPS');
        }
    }
}
